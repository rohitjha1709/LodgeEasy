...
public class NewCustomer extends JFrame {
    ...
    public NewCustomer() throws SQLException {
        ...
        c1 = new Choice();
        c1.add("Select"); // Adding default option

        try {
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select room from room where available = 'Available'");
            while (rs.next()) {
        ...
        btnNewButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    String qCheck = "select count(*) from customer where number = '" + s5 + "'";
                    ResultSet rsCheck = c.s.executeQuery(qCheck);
                    rsCheck.next();
                    if (rsCheck.getInt(1) > 0) {
                        JOptionPane.showMessageDialog(null, "ID number already exists. Please enter a unique number.");
                        return;
                    }
                    ...
                    if (e1.getMessage().contains("Duplicate entry")) {
                        JOptionPane.showMessageDialog(null, "Number Already Exists. Please enter a valid ID number.");
                    } else {
                        e1.printStackTrace();
